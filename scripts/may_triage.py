#!/usr/bin/env python3
"""May 2026 triage — clone of march_triage.py with May date window."""
...
RELEASES = json.loads((ROOT / "data" / "releases_may.json").read_text())
PRS = json.loads((ROOT / "data" / "prs_may.json").read_text())
...
def build_pr_release_index(releases) -> dict[tuple[str, int], dict]:
    ...
    for short, info in releases["repos"].items():
        for r in info["releases"]:
            if not (WINDOW_START <= r["published_at"][:10] <= WINDOW_END):
                continue
            for pr in r["prs"]:
                key = (short, pr["number"])
...
def main() -> None:
    ...
    print(f"PRs shipped in releases dated {WINDOW_START}..{WINDOW_END}: {len(pr_release_idx)}")
    ...
    (ROOT / "data" / "features_may.json").write_text(json.dumps({
        "feature_count": len(features),
        "features": features,
    }, indent=2) + "\n")

    print(f"\nMay feature groups (feat: + dedup): {len(features)}")
    ...
    SHIP_PATTERNS = [
        ...
        r"page layout", r"^add .* button", r"breadcrumb", r"column filter",
        r"Share to Slack", r"Schedule .* button", r"rollback note",
        r"View all schedules", r"restore search bar", r"instance filter",
        r"convert .* filters to API-backed", r"archive/delete",
        r"lazy session intent", r"attach report PDF",
        r"show warehouse name instead of id", r"custom date range selection",
        ...
        r"Auto Tune tab",
        r"previous_day_same_weekday", r"expand Studio data coverage",
        r"min/max savings range", r"team attribution rules",
        r"truncate schedule name", r"referral signup",
        r"enable PostHog autocapture", r"ssoUserRefreshMins",
        r"cortex functions usage", r"Databricks Job Details",
        r"redesigned? Studio", r"show user names instead of principal",
        r"email notifications for QTP", r"dbt job level enhancements",
        r"insightsection|insight section", r"streamable[ -]?http",
        r"knowledge engine|knowledge hub", r"databricks observability",
        r"databricks .* support", r"workload analytics", r"custom tag filter",
        r"week.*aggregation|weekly aggregation", r"cost attribution",
        r"multi-?connection", r"smoother streaming|auto[-\s]?resiz",
        r"InsightSection", r"export .* lineage", r"sku cost",
        r"\bcortex\b",
    ]
    ...
    print(f"\n=== MAY TRIAGE: {len(features)} features ===")
